// Live data store — replaces the design bundle's mock data.
// Fetches from /api/* on boot and every N seconds; components subscribe via useStore().
...
function notify() { STORE._listeners.forEach(fn => { try { fn(); } catch(e) {} }); }
...
function iconFor(hostName, sessionName) {
  ...
  if (host && host.config) return { icon: host.config.icon || '', color: host.config.icon_color || '' };
  return { icon: '', color: '' };
}
...
function adaptHosts() {
  return STORE.hosts.map(h => {
    ...
    return {
      ...
      // Host metrics + load history are not exposed by the backend yet (issue #20).
      // A flat zero-line placeholder keeps the sparkline renderable.
...
authFetch('/api/version').then(r => r.json()).then(v => { STORE.version = v.version || ''; notify(); }).catch(() => {});
